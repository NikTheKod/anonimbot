...
async def approve_message(callback: types.CallbackQuery):
    ...
    channel_message = (
        ...
        f"│ {message_text}\n"
        "└─────────────────┘\n\n"
        "✨ *Сообщение прислано анонимно через бота*\n"
        "💭 *Если хочешь ответить — напиши @ваш_админ*"
    )
...
async def reject_message(callback: types.CallbackQuery):
    ...
    reject_notification = (
        ...
        "📜 *Пожалуйста, ознакомься с правилами перед отправкой*\n"
        "🔁 Ты можешь отправить новое сообщение через главное меню\n\n"
        f"**Твой текст:**\n"
        f"┌─────────────────┐\n"
